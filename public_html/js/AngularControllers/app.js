...
aplicacionMundial.controller("getCompetitors", function ($http, $scope) {
    $http.get('http://localhost:8080/competitors').
    //$http.get('http://172.16.123.72:36905/co.edu.unipiloto.ws_TestWS_war_1.0-SNAPSHOT/webresources/service/getAllCompetitorsInJSON/').
...
aplicacionMundial.controller("competitorCtrl", function ($http, $scope) {
    $scope.addCompetitor = function () {
        document.write(JSON.stringify($scope.competitor));
        var url='http://localhost:8080/competitors/add';
//        $http.post('http://172.16.123.72:36905/co.edu.unipiloto.ws_TestWS_war_1.0-SNAPSHOT/webresources/service/agregarPaciente/',
$http.post(url,
                $scope.competitor).success(function (data, headers) {
            $scope.competitor = {};
